...
export class RitualEngine {
  static calculateResult(items) {
    ...
    const totalPower = items.reduce(
      (sum, item) => sum + (item.ritualValue || 0),
      0,
    );
    ...
    const possibleWeapons = Object.values(Definitions.weapons).filter(
      (w) => w.ritualValue !== undefined,
    );
    ...
    const baseWeapon = possibleWeapons[0];
